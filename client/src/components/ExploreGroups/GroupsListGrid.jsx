...
import {
  ...
  const GroupsListGrid = (props) => {
    ...
    return (
      <div className='group-cont mx-auto'>
        <div className='group-list container d-flex justify-content-center'>
            <div className={props.groups.length < 3 ? 'list-group flex-md-row': 'row mt-3'} >
              {props.groups.map((group, i) => {
                group.category = group.category.slice(0, 1).toUpperCase() + group.category.slice(1);
                return (
                  <div key={i} className='col-xs-3 offset-*'>
